...
while(True):
    for machine in machines_data:
        if(running[machine['machine_id']]==[]):
            for wafer in wafers:
                if(wafers[wafer][machine['step_id']]!=0 and check(wafer,finished[machine['machine_id']],running)):
                    name=wafer
            ...
            if(running[machine['machine_id']][1]-1==time):
                finished[machine['machine_id']].append(running[machine['machine_id']][0])
                running[machine['machine_id']]=[]
    time+=1
    flag=1
    for machine in machines_data:
        if(len(finished[machine['machine_id']])!=quantity):
            flag=0
            break
    if(flag==1):
        break
...
with open('Milestone0Output.json','w') as file:
