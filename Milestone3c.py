...
with open('Input/Milestone3c.json','r') as file:
    data=json.load(file)
...
while(True):
    ...
    for machine in machines_data:
        if(running[machine['machine_id']]==[] and check_parameters(machine['step_id'],steps_data,machine['machine_id'],machine_parameters[machine['machine_id']]) and fluctuation_cooldown[machine['machine_id']]<=time):
            for wafer in wafers:
                if(machine['step_id'] in wafers[wafer].keys() and wafers[wafer][machine['step_id']]!=0 and check(wafer,finished[machine['step_id']],running) and wafer not in current_finished):
                    name=wafer
...
with open('Milestone3cOutput.json','w') as file:
